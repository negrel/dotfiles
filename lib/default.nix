...
rec {
  ...
  isEmpty = (
    a:
    if builtins.isAttrs a then (builtins.length (lib.attrValues a) == 0) else (builtins.length a) == 0
  );
  ...
  listToAttrsRecursive = (
    ...
    lib.foldr (
      ...
      acc
      // {
        "${el.name}" = if builtins.isList el.value then listToAttrsRecursive el.value else el.value;
      }
    ) { } list
  );
  ...
  listRegularFiles = (
    dir:
    lib.mapAttrs (
      key: value: if value == "directory" then listRegularFiles (dir + "/${key}") else value
  ...
  listNixFiles = (
    ...
    let
      result = lib.filterAttrsRecursive (
        key: value: if builtins.isAttrs value then true else lib.hasSuffix ".nix" key
      ) (listRegularFiles dir);
    in
    lib.filterAttrsRecursive (
      key: value: if builtins.isAttrs value then !isEmpty value else true
    ) result
  );
    ...
    let
      recurse = (
        ...
        let
          ...
          mappedList = lib.foldr (
            v: acc:
            acc
            ++ [
              {
                name = f v.name v.value;
                value = if builtins.isAttrs v.value then recurse v.value else v.value;
              }
            ]
          ) [ ] list;
        in
      ...
      recurse =
        ...
        lib.mapAttrs (
          ...
          if builtins.isAttrs value then
            ...
            if lib.hasAttrByPath [ "default" ] value then
              recursiveValue // (pkgs.callPackage (dir + "/${name}/default.nix") callInputs)
            else
              recursiveValue
          else
            pkgs.callPackage (dir + "/${name}.nix") callInputs
        ) files;
    in
    lib.fixedPoints.fix (
      self:
      let
        ...
        files = mapAttrsNameRecursive (name: value: lib.removeSuffix ".nix" name) (listNixFiles dir);
      in
  ...
  readSecret = name: builtins.replaceStrings [ "\n" ] [ "" ] (readSecretFile name);

  readSecretFile = name: builtins.readFile (../secrets + "/${name}");
}
